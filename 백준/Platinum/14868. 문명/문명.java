...
public class Main{
	...
	static int isJoint(int y, int x){
		for(int i =0; i<4; i++){
			...
			if(board[yy][xx] != 0) return board[yy][xx];
		}
		return 0;
	}
	...
	static int bfs(){
		while(!q.isEmpty()){
			Pair now = q.poll();
			int pn = find(board[now.y][now.x]);
			for(int i =0; i<4; i++){
				int y = dy[i] + now.y;
				int x = dx[i] + now.x;
				if(!isValid(y, x) || find(board[y][x]) == pn) continue;
				...
				if(cnts[find(board[y][x])] == k){
					if(time[y][x] == now.t+1) return now.t+1;
					else return now.t;
				} 
			}
		}
